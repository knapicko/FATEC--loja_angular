...
export class ProdutoDetalhe {
  ...
  constructor(private route: ActivatedRoute) {
    ...
    this.produto = this.listaProdutos.find(p => p.codigo === codigo) || null;
    this.produtoNaoEncontrado = !this.produto;
  }

  aumentarQuantidade() {
    if (this.produto && this.quantidade < this.produto.quantidade) {
      this.quantidade++;
    }
  }
  ...
  get precoFinal() {
    ...
    const preco = this.produto.promo < this.produto.valor ? this.produto.promo : this.produto.valor;
    return preco * this.quantidade;
  }

  get descontoPercentual() {
    if (!this.produto || this.produto.promo >= this.produto.valor) return 0;
    return Math.round((1 - this.produto.promo / this.produto.valor) * 100);
  }

  adicionarCesta() {
    alert(`Produto "${this.produto?.nome}" adicionado à cesta!\nQuantidade: ${this.quantidade}\nTotal: R$ ${this.precoFinal.toFixed(2)}`);
  }
}